...
func _crear_nivel(numero_nivel:int):
	...
	for i in hijos.size():
		...
		if hijos[i].is_in_group("jefes"):
			hijos[i].jefe_muerto.connect(siguiente_nivel)
			hijos[i].jefe_danado.connect(_on_jefe_danado)
...
func _on_jefe_danado(_vida_restante: int) -> void:
	# Aquí actualizas tu barra de vida del jefe si tienes UI
	pass
